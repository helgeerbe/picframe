...
class EnvironmentBootstrapper:
    ...
    def __init__(
        ...
        self.config_db_path = (
            Path(config_db_path) if config_db_path else self.data_dir / "config.db3"
        )
        self.media_db_path = (
            Path(media_db_path) if media_db_path else self.data_dir / "media_cache.db3"
        )
        self.force = force
    ...
    def _prompt_deletion(self, filepath: Path, db_name: str) -> bool:
        ...
        prompt = (
            ...
            "Do you want to delete it and start fresh? [y/N]: "
        )
        ...
        if response in ["y", "yes"]:
            ...
            logger.info(
                f"Keeping existing {db_name} database. Migrations will be applied if necessary."
            )
            return False
    ...
    def bootstrap(self) -> None:
        ...
        self._prompt_deletion(self.media_db_path, "Media Cache")
        ...
        SQLiteMediaRepository(str(self.media_db_path))
    ...
    def _copy_assets(self) -> None:
        ...
        if not pkg_data_dir.exists():
            logger.warning(
                f"Package data directory not found at {pkg_data_dir}. Skipping asset copy."
            )
            return
