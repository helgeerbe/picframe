...
def get_hardware_decoders_caps() -> list[Any]:
    """
    Queries the GStreamer registry for hardware video decoders and returns
    their sink pad template caps.
...
def is_hardware_supported(media_caps_str: str) -> bool:
    ...
    media_caps = Gst.Caps.from_string(media_caps_str)
    if not media_caps:
        return False
...
def find_best_element(element_types: list[str]) -> str | None:
    """
