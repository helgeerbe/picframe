...
class ImageMetadataStrategy(IMetadataStrategy):
    ...
    def extract(self, filepath: str, directory_id: int) -> MediaItem | None:
        ...
        try:
            ...
            caption = iptc_data.get("caption") or xmp_data.get("caption") or exif_data.get("caption")
                ...
                if "GPS GPSLatitude" in tags and "GPS GPSLatitudeRef" in tags and \
                   "GPS GPSLongitude" in tags and "GPS GPSLongitudeRef" in tags:
                    try:
                        ...
                        if isinstance(lat_val, list) and len(lat_val) == 3 and \
                           isinstance(lon_val, list) and len(lon_val) == 3:
                            
                            lat = float(lat_val[0].num) / float(lat_val[0].den) + \
                                  (float(lat_val[1].num) / float(lat_val[1].den)) / 60.0 + \
                                  (float(lat_val[2].num) / float(lat_val[2].den)) / 3600.0
                            if lat_ref == 'S':
                                lat = -lat
                                
                            lon = float(lon_val[0].num) / float(lon_val[0].den) + \
                ...
                if iptc:
                    ...
                    if (2, 25) in iptc:
                        ...
                        if isinstance(keywords, list):
                            iptc_data["tags"] = ", ".join(k.decode('utf-8', errors='ignore') if isinstance(k, bytes) else str(k) for k in keywords)
            ...
            with Image.open(filepath) as img:
                if hasattr(img, 'getxmp'):
                    ...
                    if xmp:
                        ...
                        if val and isinstance(val, dict) and 'Alt' in val and 'li' in val['Alt'] and 'text' in val['Alt']['li']:
