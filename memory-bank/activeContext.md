...
- Ticket #690 fixes the Settings display-geometry apply path by splitting renderer config changes: backend toggles require service restart, shader/font changes rebuild renderer components, image-prep settings remain live, and GNOME/VM-style Wayland fullscreen-host geometry changes rebuild image/text/clock components against the existing display surface and schedule a fresh media render.
