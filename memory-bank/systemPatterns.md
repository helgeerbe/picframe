...
- The First/Last Frame Sandwich pattern hides GStreamer startup/shutdown artifacts:
  ...
  - Use the cached first frame as an opaque GTK host backdrop when sidecar metadata marks matted or edge-filled frames, including on Raspberry Pi/labwc, so non-video regions show the intended mats/bars instead of stale pi3d pixels.
...
- On Wayland, require the GTK4 `playbin`/GTK-compatible `gtk4paintablesink` presentation path inside a borderless fullscreen GTK4 host. Raspberry Pi/labwc uses a transparent host only when no cached backdrop is supplied; matted/edge-filled handoffs use an opaque fixed host with the cached frame behind the live paintable. GNOME/VM uses an opaque host colored from `viewer.background`. The host/backdrop covers the display rectangle; the live paintable uses sidecar `content_rect` when transition-frame metadata is valid, and explicit `content_fit` controls fill/contain behavior. The GTK4 video surface hides the cursor during playback.
