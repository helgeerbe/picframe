...
- Non-shuffle resume (#786): `PlaylistManager` persists the primary media id of each displayed slot to `model.resume_media_id` in `config.db3` only while `shuffle is False`. On the first non-shuffle build after startup it advances the cursor past the marked slot (slot+1, wrapping to 0, falling back to 0 if the item is gone); the marker is one-shot (`_resume_applied`) so config-change rebuilds and `restart_playlist` don't re-jump. Keying on media id (not raw index) keeps resume robust against file additions/reordering. `Command.RESTART_PLAYLIST` (`PlaybackEngine._handle_restart_playlist_command`) clears the marker, drops history, rebuilds with the current shuffle setting, and re-triggers the first item; the Remote shuffle dropdown exposes it as an adaptive "Restart playlist" / "Reshuffle now" action.
