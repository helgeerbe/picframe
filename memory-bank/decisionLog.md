...
- Treat the generated video transition-frame sidecar as the live video geometry contract: `frame_size`, `coordinate_space=frame_pixels`, and `content_rect` define the live video window for every generated frame type. GTK host backdrops are visual only; explicit `content_fit` carries fill/contain intent through IPC and pipeline construction.
