...
- Non-shuffle resume (#786): resume keys on media id (not raw index) in `model.resume_media_id` in `config.db3`, written only while `shuffle is False` (shuffled order is unstable across restarts); on the first non-shuffle build after startup the cursor advances past the marked slot (one-shot `_resume_applied` so config-change rebuilds don't re-jump); `Command.RESTART_PLAYLIST` clears the marker, drops history, rebuilds with the current shuffle setting, and re-triggers the first item; the Remote shuffle dropdown exposes it as an adaptive "Restart playlist" (shuffle off) / "Reshuffle now" (shuffle on) action.
