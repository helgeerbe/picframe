...
- Fullscreen video fills the GTK4 host. Raspberry Pi/labwc uses a transparent fullscreen host for plain videos, but matted/edge-filled videos with cached backdrop metadata use an opaque fixed host with the cached frame under the live paintable. GNOME/VM uses an opaque fullscreen host colored from `viewer.background`. Custom non-fullscreen video places the paintable at the renderer-reported `viewer.display_x/y/w/h` rectangle.
