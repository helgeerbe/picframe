...
- Ticket #694 brightness hardening is implemented locally: internal display outputs use `brightnessctl`, HDMI/DDC brightness probes VCP 0x10 before writing, command output is included in `brightness_unavailable` errors, repeated identical failures are suppressed, Remote brightness drags preview locally and commits one hardware command on release/change, and docs explain DDC prerequisites plus monitor brightness floors.
...
- Latest backend verification on `v2-dev` (2026-06-19): `.venv/bin/python -m pytest` passed with 632 tests and 1 GI deprecation warning.
