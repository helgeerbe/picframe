...
- #693 is implemented and verified locally; push and GitHub issue closeout remain pending until the local commits are published.
...
- Latest backend verification on `v2-dev` (2026-06-19): `.venv/bin/python -m pytest` passed with 626 tests and 1 GI deprecation warning.
- Latest local merge verification on `v2-dev` (2026-06-18): `npm run build` passed, with only sandbox stream-fd warnings before Vite completed.
