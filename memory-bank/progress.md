...
- Video transition-frame visual parity: cached first/last frames now honor viewer matting, `blur_edges`, `edge_alpha`, and `background` settings; managed cache filenames include processing hashes and legacy sidecars validate `.meta.json` signatures before reuse.
