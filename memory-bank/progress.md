...
- Ticket #695 clock hour mode is implemented locally: Settings exposes 24-hour, 12-hour, and Custom clock choices while persisting only `viewer.clock_format`; locale does not drive clock hour mode.
...
- Latest backend verification on `v2-dev` (2026-06-19): `.venv/bin/python -m pytest` passed with 634 tests and 1 GI deprecation warning.
