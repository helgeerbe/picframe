...
*   **Video Transition Frame Visual Parity:** Cached video first/last frames are generated with the same visual edge intent as still images. `viewer.blur_edges` builds display-sized blurred-edge composites; otherwise `viewer.edge_alpha` blends image-derived edge fill with `viewer.background`. If matting, blur, or image-derived edge fill creates backdrop pixels, playback metadata marks the cached first frame as a GTK host backdrop while keeping live video geometry unchanged except for true matted content rects.
*   **Video Transition Cache Signatures:** Managed cache filenames include a short SHA-256 digest over source path, source size/mtime, display size, `viewer.video_fit_display`, background, matting settings/resource signature, edge-fill settings, and frame role. Legacy sidecar paths (`.1.frame` / `.2.frame`) keep their stable names but require matching `.meta.json` processing signatures before reuse, so stale frames regenerate automatically after visual setting changes.
