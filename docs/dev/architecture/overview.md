...
*   **Video Transition Frame Visual Parity:** Cached video first/last frames are generated with the same visual edge intent as still images. `viewer.blur_edges` builds display-sized blurred-edge composites; otherwise `viewer.edge_alpha` blends image-derived edge fill with `viewer.background`. Each generated frame sidecar stores `frame_size`, `coordinate_space=frame_pixels`, and the visible video-opening `content_rect`; playback uses that rect for the live video window for matted, solid-bar, edge-filled, blurred, and `video_fit_display` frames. Beveled mat effects can visually overlap cached frame pixels, so they intentionally inset the live video placement rectangle to keep frame shadows visible. If first-frame image loading falls back to direct playback while valid sidecar metadata is available, playback still uses `content_rect` instead of the full display rectangle. If matting, blur, or image-derived edge fill creates backdrop pixels, playback metadata marks the cached first frame as a GTK host backdrop, and the GTK presenter uses an opaque fixed host for that backdrop even on Raspberry Pi/labwc.
