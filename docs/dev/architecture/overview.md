...
*   **Reasoning & Benefits:** Prevents the creation of a "God Object." The `PlaylistManager` can be tested purely for its logic (e.g., "does shuffle work?"), while the `PlaybackEngine` can be tested purely for its state transitions (e.g., "does it pause correctly?"), reducing cognitive load and test complexity. `State.PAUSED` is the explicit pause state published to Remote, MQTT, and WebSocket clients; `IDLE` is not used as a user-facing pause alias. Pausing also sends a renderer status overlay: pi3d draws `PAUSED` for still images and freezes in-flight image/video-title-card fades, while active videos receive a GTK video-window pause label because the video window is composited above pi3d. Active video resume is an in-place GStreamer pipeline state change, so a paused clip continues from the paused timestamp instead of replaying the handoff.
...
*   **Reasoning & Benefits:** Prevents lifecycle conflicts. Rebuilding the media cache (which happens frequently) will never risk corrupting user configuration. The Repository Pattern abstracts the SQLite implementation, allowing the business logic to interact with simple Python objects rather than SQL queries. Both SQLite repositories serialize access to their shared connections because playback, API, MQTT, logging, hardware, indexing, and geocoding can touch persistent storage from different threads. Media selection uses indexable filepath ranges, active-media/date indexes, rounded-coordinate location indexes, Python-side standard shuffle, and count queries that skip resolved-location joins unless location filters require them.
...
*   **System Management & Error Handling:** The `SystemManager` service delegates critical OS commands (reboot, shutdown) to the injected `ISystemManager` adapter. For Linux environments, the `LinuxSystemManager` utilizes `subprocess` to execute exact-path `sudo -n` commands so missing sudoers permissions fail immediately instead of hanging on an interactive password prompt. The API can also inspect `picframe.service` and restart it noninteractively for settings that are explicit service boundaries, such as `viewer.use_glx` and `viewer.use_sdl2`; if the service is inactive, systemd is unavailable, or sudoers is missing, Settings reports that a manual restart is required. To ensure system resilience and user visibility, these adapters must implement robust error handling. If a system command fails (e.g., due to missing `sudo` privileges), the adapter should catch the exception and publish or return an explicit system error/manual-required result rather than failing silently.
*   **GStreamer Hardware Decoding & VM Fallback:** The `GstVideoRenderer` leverages an out-of-process worker (`gst_worker.py`) communicating via IPC to dynamically construct pipelines. The worker delegates decoder policy to `gst_playback_policy.py`, GTK4 window/picture handling to `gtk_video_presenter.py`, and GTK pipeline construction to `gst_pipeline_builder.py`. It automatically utilizes hardware decoding (VA-API, V4L2) when available, and uses bounded software decoding (`libav`) on environments such as Ubuntu VMs with `virtio-gpu` when the configured software limit allows it. Raspberry Pi HEVC Main10/HDR remains hardware-only and is skipped cleanly if a safe hardware path is unavailable.
*   **GTK-backed Wayland Video Handoff:** On Wayland, GTK4 `gtk4paintablesink` presentation is mandatory. The `GstVideoRenderer._worker_environment()` method enforces `GDK_BACKEND=wayland` in the worker subprocess so GTK4 never falls back to X11/Xwayland (which causes green-screen / segfault crashes on Raspberry Pi 5 under labwc — see #710). When `WAYLAND_DISPLAY` is not set, the renderer dynamically scans `XDG_RUNTIME_DIR` for a single `wayland-*` Unix-domain socket and uses it; if none or multiple are found, a warning is logged. The worker also logs display-related environment variables (`WAYLAND_DISPLAY`, `DISPLAY`, `XDG_RUNTIME_DIR`, `GDK_BACKEND`) before GTK4 initialization so silent crashes leave a diagnostic trail. Raspberry Pi/labwc uses a transparent fixed GTK4 host for fullscreen plain videos so the worker can place a pause-status label above the live paintable; inset or custom video rectangles use an opaque fixed host, with the cached first frame behind the live paintable when a sidecar backdrop is available and `viewer.background` as fallback. GNOME/VM uses an opaque fullscreen host colored from `viewer.background` so desktop shell UI cannot show through. The GTK host covers the configured display rectangle or fullscreen window; when a valid transition-frame sidecar is available, the live video paintable uses the sidecar `content_rect` so it matches the visible video opening inside mats, solid bars, edge fill, or blurred backdrops. At EOS, the worker dims the GTK4 window to 99% opacity so pi3d can redraw behind it before the video window is destroyed. If GTK4 or `gtk4paintablesink` is unavailable, the worker publishes a GTK presentation system error instead of falling back to legacy sinks.
*   **Video Title-Card Timing:** The cached first video frame is rendered with a video-specific first-frame action before GStreamer starts. If overlay text is generated, pi3d waits through text fade-in, `viewer.show_text_tm`, fade-out to zero alpha, and clean redraw frames before publishing the transition completion that starts video. When `viewer.show_text_on_video` is `False` (the default), the playback engine suppresses the metadata text overlay for video media so GStreamer starts immediately without waiting for the `show_text_tm` fade-out; the clock overlay is preserved and status overlays are preserved through `dataclasses.replace()` (issue #726). Manual next/previous navigation uses the same handoff path as timed playback. Video first-frame render commands carry a transition token so stale completions from interrupted handoffs cannot start the wrong video. If playback is paused during this first-frame handoff, pi3d freezes the title-card fade and the playback engine defers the GStreamer `PLAY` command until playback resumes. The cached last-frame reveal is promoted only after the worker reports that the sink has rendered a real video frame when that statistic is available.
*   **Video Transition Frame Visual Parity:** Cached video first/last frames are generated with the same visual edge intent as still images. `viewer.blur_edges` builds display-sized blurred-edge composites; otherwise `viewer.edge_alpha` blends image-derived edge fill with `viewer.background`. Each generated frame sidecar stores `frame_size`, `coordinate_space=frame_pixels`, and the visible video-opening `content_rect`; playback uses that rect for the live video window for matted, solid-bar, edge-filled, blurred, and `video_fit_display` frames. Beveled mat effects can visually overlap cached frame pixels, so they intentionally inset the live video placement rectangle to keep frame shadows visible; for video transition frames only, source-influenced pixels outside `content_rect` are rendered from a black source so the gap does not freeze a video edge. If first-frame image loading falls back to direct playback while valid sidecar metadata is available, playback still uses `content_rect` instead of the full display rectangle. If matting, blur, or image-derived edge fill creates backdrop pixels, playback metadata marks the cached first frame as a GTK host backdrop, and the GTK presenter uses an opaque fixed host for that backdrop even on Raspberry Pi/labwc.
*   **Video Transition Cache Signatures:** Managed cache filenames include a short SHA-256 digest over source path, source size/mtime, display size, `viewer.video_fit_display`, background, matting settings/resource signature, edge-fill settings, frame role, and processing version. Legacy sidecar paths (`.1.frame` / `.2.frame`) keep their stable names but require matching `.meta.json` processing signatures and current geometry metadata before reuse, so stale frames regenerate automatically after visual setting or geometry-contract changes.
...
*   **Indexing-Time Date Fallback:** `ImageMetadataStrategy` and `VideoMetadataStrategy` extract the date from EXIF metadata (images) or container creation date (videos) during indexing. If no date is found in the media metadata, the strategy falls back to the file's `last_modified` timestamp at indexing time so the media cache always has a valid `exif_datetime` value. This keeps date-range filters and date ordering in Remote working for all media, including scanned images or video clips that lack embedded dates.
...
*   **Reasoning & Benefits:**
    ...
    *   **Overlay Z-Order Convention:** The text/clock overlay stack draws its gradient sprite *behind* the text so the gradient no longer occludes the text. The convention is: status overlay `z=0.05` (front), text `z=0.1`, gradient `z=0.3` (behind text), image `z=5.0`. The gradient uses a full-render-height texture scaled on the GPU via `sprite.scale()` so it is not rebuilt when `viewer.text_bkg_hgt` changes. The gradient sprite must set `z` via `position()` only (not the constructor), pass numpy arrays directly to `pi3d.Texture` (no PIL conversion), and use the same display instance as the other overlay components.
...
*   **Reasoning & Benefits:**
    *   **Pre-roll Extraction:** During the initial file scanning process (`MediaIndexerService` / `VideoMetadataStrategy`), the system extracts the first and last frames of the video and caches them locally (`.1.frame`, `.2.frame`) or in the managed runtime cache. The cached pixels include the configured transition-frame matting/edge-fill processing, and the sidecar records the generated-frame geometry used to place live video. An on-demand fallback regenerates the frames immediately if they are missing, their processing signature is stale, or the sidecar is missing current geometry metadata during playback.
