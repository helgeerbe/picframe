...
flowchart TB
    ...
    Gst -->|gtk4paintablesink or fallback sink| GtkSink
```
...
*   **GTK-backed Wayland Video Handoff:** On Wayland, the worker prefers a `playbin` + `gtk4paintablesink` presentation path hosted in a borderless transparent GTK4 window. Fullscreen video fills the host; custom `viewer.display_x/y/w/h` geometry uses a fullscreen transparent host with the video paintable placed at the configured rectangle. At EOS, the worker dims the GTK4 window to 99% opacity so pi3d can redraw behind it before the video window is destroyed. If GTK4, `gtk4paintablesink`, or geometry confirmation fails, the worker preserves the existing `waylandsink` render-rectangle path as the fallback.
