...
*   **Concept:** The system employs a strict separation between how configuration is stored (flat key-value pairs in SQLite) and how it is presented/consumed by the frontend (nested JSON objects). A dedicated Service Layer (`ConfigService`) acts as an Anti-Corruption Layer, handling the flattening and unflattening of data. Furthermore, Pydantic models are used at the API boundary to strictly validate incoming configuration payloads against the expected schema.
*   **Reasoning & Benefits:** This enforces Separation of Concerns (SoC). The API layer (`app.py`) is relieved of data transformation logic, remaining a thin HTTP wrapper. The Service Layer encapsulates the business rules for data mapping, making it highly testable in isolation. Pydantic validation ensures that the database is never polluted with invalid or obsolete configuration keys sent by the frontend, maintaining strict alignment with the `configSchema.json` contract.
