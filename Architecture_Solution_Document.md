...
5.  **System Permissions:** For target hardware deployments (e.g., Raspberry Pi), the user executing the `picframe` application must have passwordless `sudo` privileges configured for specific system commands, notably `/sbin/reboot` and `/sbin/shutdown`, to allow the `LinuxSystemManager` to function correctly without interactive prompts.