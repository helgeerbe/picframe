...
const colorValue = computed(() => {
  if (Array.isArray(props.modelValue) && props.modelValue.length >= 3) {
    return `#${props.modelValue.slice(0, 3).map(value => Number(value).toString(16).padStart(2, '0')).join('')}`
  }
...
<template>
  <div class="flex flex-wrap items-center gap-3">
    <button
      type="button"
      :class="[!enabled ? 'bg-indigo-600 text-white' : 'border border-gray-300 text-gray-700 dark:border-gray-600 dark:text-gray-300', 'rounded-md px-3 py-2 text-sm font-medium']"
      @click="setAuto"
    >
      Auto
    </button>
