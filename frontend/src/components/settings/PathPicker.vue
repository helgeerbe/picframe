...
async function validatePath() {
  ...
  try {
    ...
    validation.value = {
      ...
      is_file: false,
      warnings: [],
      error: error?.response?.data?.detail || error?.message || 'Path is not allowed'
    }
  }
}
...
async function browse(path = props.modelValue || '~') {
  ...
  try {
    ...
    browseError.value = error?.response?.data?.detail || error?.message || 'Could not browse path'
    browseState.value = await configStore.browseFilesystem({
      path: '~',
      kind: props.kind,
      extensions: props.extensions
...
<template>
  <div class="space-y-2">
    ...
    <p v-else-if="validation?.valid" class="text-xs text-gray-500 dark:text-gray-400">{{ validation.exists ? 'Path exists' : 'Path will be created or mounted later' }}</p>
    ...
    <div v-if="browseOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-gray-900/60 p-4">
      <div class="max-h-[85vh] w-full max-w-3xl overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-800">
        <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-700">
          <div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Select path</h3>
        ...
        <div class="grid max-h-[70vh] grid-cols-1 overflow-y-auto md:grid-cols-[12rem_minmax(0,1fr)]">
          <aside class="border-b border-gray-200 p-3 dark:border-gray-700 md:border-b-0 md:border-r">
            <p class="mb-2 text-xs font-semibold uppercase text-gray-500">Shortcuts</p>
            <button
