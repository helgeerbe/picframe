...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-3 border-b border-gray-100 pb-5 dark:border-gray-700/50 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ t('settings.hardwareInputs.enabled') }}</h3>
      </div>
      <button
        type="button"
        @click="updateEnabled(!config.enabled)"
        :class="[config.enabled ? 'bg-indigo-600' : 'bg-gray-200 dark:bg-gray-700', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2']"
      >
        <span :class="[config.enabled ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow transition']"></span>
      </button>
    </div>
    ...
    <div v-if="rows.length === 0" class="rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
          ...
          <tr v-for="[id, mapping] in rows" :key="id" class="align-top">
            ...
            <td class="py-4 pr-4">
              <input
                ...
                :class="[hasDuplicatePin(mapping.pin) ? 'border-red-500 text-red-600 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600', 'block w-24 rounded-lg bg-white px-3 py-2 text-gray-900 shadow-sm dark:bg-gray-700 dark:text-white']"
              >
              <div v-if="hasDuplicatePin(mapping.pin)" class="mt-1 text-xs font-medium text-red-600">
            ...
            <td class="py-4 pr-4">
              <div class="space-y-2">
                ...
                >
                  <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t(`settings.hardwareInputs.action.${action}`) }}</span>
                  <select
                  ...
                  >
                    ...
                    <option v-for="command in commandOptions" :key="command" :value="command">{{ command }}</option>
                  </select>
                </label>
                <label
                  ...
                  class="grid grid-cols-[8rem_minmax(10rem,1fr)] items-center gap-2"
                >
                  <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('settings.hardwareInputs.noMotionDelay') }}</span>
                  <input
                    type="number"
                    min="0"
                    step="1"
                    :value="mapping.no_motion_delay_seconds ?? 0"
                    @input="updateInput(id, { no_motion_delay_seconds: Number(($event.target as HTMLInputElement).value) })"
                    class="block rounded-lg border-gray-300 bg-white px-3 py-2 text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                  >
                </label>
              </div>
