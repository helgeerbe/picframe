...
def test_geocoding_worker_start_stop(
    mock_geo_reverse_class: MagicMock, mock_repo: MagicMock, mock_config_repo: MagicMock
) -> None:
    worker = GeocodingWorker(mock_repo, mock_config_repo)
    worker.start()
...
@patch("picframe.core.services.geocoding_worker.GeoReverse")
def test_geocoding_worker_queue_coordinates(
    mock_geo_reverse_class: MagicMock, mock_repo: MagicMock, mock_config_repo: MagicMock
) -> None:
    worker = GeocodingWorker(mock_repo, mock_config_repo)
    worker.start()
...
@patch("picframe.core.services.geocoding_worker.GeoReverse")
def test_geocoding_worker_process_queue(
    mock_geo_reverse_class: MagicMock, mock_repo: MagicMock, mock_config_repo: MagicMock
) -> None:
...
@patch("picframe.core.services.geocoding_worker.GeoReverse")
def test_geocoding_worker_cache_hit(
    mock_geo_reverse_class: MagicMock, mock_repo: MagicMock, mock_config_repo: MagicMock
) -> None:
    mock_geo_instance = MagicMock()
    mock_geo_reverse_class.return_value = mock_geo_instance
