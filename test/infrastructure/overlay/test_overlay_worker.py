...
def test_build_shell_config_effective_layout_merges_db_overrides(tmp_path) -> None:
    ...
    assert plugin["layout"]["width"] is None
    assert plugin["layout"]["height"] is None
    assert plugin["layout"]["scale"] == 1.0
    # #752: the manifest design size is forwarded so the shell can scale the
    # widget iframe (scale mode).
...
def test_install_output_loss_detection_connects_monitor_removed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``_install_output_loss_detection`` wires ``monitor-removed`` on the window's
    GdkDisplay so output destruction self-reports to the main process (#755)."""
    import picframe.infrastructure.overlay.overlay_worker as mod

    monkeypatch.setattr(mod, "WEBKIT_AVAILABLE", True)

    worker = make_worker()
    worker._loop = MagicMock()  # production path
    fake_display = MagicMock()
    window = MagicMock()
    window.get_display.return_value = fake_display
    worker._window = window

    worker._install_output_loss_detection()

    fake_display.connect.assert_called_once_with("monitor-removed", worker._on_monitor_removed)


def test_install_output_loss_detection_noop_in_headless(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Without a GLib loop (headless/unit tests) nothing is connected."""
    import picframe.infrastructure.overlay.overlay_worker as mod
    ...
    worker._window.get_display.assert_not_called()


def test_install_output_loss_detection_noop_when_display_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A window with no GdkDisplay is skipped without raising (#755 defensive)."""
    import picframe.infrastructure.overlay.overlay_worker as mod
    ...
    worker._install_output_loss_detection()  # must not raise


def test_on_monitor_removed_emits_surface_orphaned_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``monitor-removed`` emits a single ``SurfaceOrphanedEvent`` per cycle (#755)."""
    import picframe.infrastructure.overlay.overlay_worker as mod
    ...
    worker = make_worker()
    sent: list[Any] = []
    monkeypatch.setattr(worker, "_send_event", lambda evt: sent.append(evt))

    worker._on_monitor_removed(MagicMock(), MagicMock())
    assert len(sent) == 1
    assert isinstance(sent[0], SurfaceOrphanedEvent)

    # A burst (the ~1 s HPD blip) must collapse to one report.
    worker._on_monitor_removed(MagicMock(), MagicMock())
    assert len(sent) == 1


def test_on_monitor_removed_resets_reported_on_fresh_surface(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A fresh ``_build_surface`` re-arms the once-per-cycle report (#755).

    After the renderer respawns the worker, the next power cycle must report
    again, so a single report flag is not sticky across the worker's lifetime.
    """
    import picframe.infrastructure.overlay.overlay_worker as mod
    ...
    # Re-arm: a fresh surface build resets the flag.
    worker._output_lost_reported = False
    worker._on_monitor_removed(MagicMock(), MagicMock())
    assert len(sent) == 2
